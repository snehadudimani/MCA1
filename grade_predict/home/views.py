...
def grade_backend(request):
    ...
    age = int(dict1.get('age', 0))  # Get 'age' field and convert to int
    absences = int(dict1.get('absences', 0))  # Get 'absences' field and convert to int

    # Validate age and absences
    if not (1 <= age <= 50):
        return HttpResponse("Invalid age. Age must be between 1 to 50.")

    if not (0 <= absences <= 200):
        return HttpResponse("Invalid absences. Absences must be between 0 to 200.")
